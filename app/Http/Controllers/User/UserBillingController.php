...
class UserBillingController extends Controller
{
    ...
    public function showStatementDetails(Statement $statement): View
    {
        ...
        $statementDetails = [
            'statementDate' => $statement->formatted_statement_date,
            'accountIdentifier' => $statement->user->service_type ?? ($statement->user->account_number ?? 'Account'),
            'servicePeriod' => ($statement->service_period_start && $statement->service_period_end)
                ? (\Carbon\Carbon::parse($statement->service_period_start)->format('M j') . ' – ' . \Carbon\Carbon::parse($statement->service_period_end)->format('M j, Y'))
                : 'N/A',
            'amountDue' => (float) $statement->total_amount,
            'paymentDueDate' => $statement->formatted_due_date,
            'accountHolder' => [
                'name' => $statement->user->display_name ?? trim(($statement->user->first_name ?? '') . ' ' . ($statement->user->last_name ?? '')),
                'addressLine1' => $statement->user->address ?? '',
                'addressLine2' => trim(($statement->user->city ?? '') . ', ' . ($statement->user->state ?? '') . ' ' . ($statement->user->zip_code ?? '')),
                'accountNumber' => $statement->user->account_number ?? 'N/A',
            ],
            'previousActivity' => [
                'remainingBalance' => (float) (($statement->previous_balance ?? 0) - ($statement->payments_received ?? 0)),
            ],
            'currentActivity' => $statement->items->map(function ($item) {
                return [
        ...
        return view('user.billing.statement-detail', compact('statementDetails'));
    }
    ...
    public function downloadStatementPdf(Statement $statement) // Returns \Symfony\Component\HttpFoundation\Response
    {
        ...
        $dataToPassToView = [
            ...
            'barcodeDataStringForDisplayLine1' => $barcodeDataStringForDisplay,
            'barcodeDataStringForDisplayLine2' => $barcodeDataStringForDisplay,
        ];
